...
native-OFF); AVX2 hw (oracle-air) runs .185 fine; the cost is hypothesized to be the per-op
...
build.sh's `[6x]` steps. The full silicon suite runs on **oracle-air** before shipping.
...
**oracle-air** (`sh build.sh` full) must be green before any ship.
