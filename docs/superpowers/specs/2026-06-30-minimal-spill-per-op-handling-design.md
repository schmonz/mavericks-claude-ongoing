...
.185 on AVX2 hardware (oracle-air, our baseline) starts fine. Measurement established:
...
trusted .185 startup on the target reaches idle in a few seconds (oracle-air-grade), in a
...
- **Correctness:** oracle-gate every live-register lowering vs `bmi_exec`/`vec_exec`
  ...
  differential suite (`build.sh`) on **oracle-air** before shipping any dylib. Keep the
